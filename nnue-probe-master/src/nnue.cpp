    ...
    const DirtyPiece *dp, IndexList *removed, IndexList *added)
{
  int ksq = pos->squares[c];
  ksq = orient(c, ksq);
  const unsigned max_vals = sizeof(removed->values) / sizeof(removed->values[0]);
  const unsigned pieceTableSize = sizeof(PieceToIndex[0]) / sizeof(PieceToIndex[0][0]);

  for (int i = 0; i < dp->dirtyNum; i++) {
    int pc = dp->pc[i];

    // Validate piece code
    if (pc < 0 || (unsigned)pc >= pieceTableSize)
      continue;
  ...
  for (unsigned c = 0; c < 2; c++) {
    ...
    for (size_t k = 0; k < activeIndices[c].size; k++) {
      ...
      if (index >= FtInDims) {
        ...
        if (!warned) {
          warned = true;
          fprintf(stderr, "nnue: refresh_accumulator skipping invalid index %u (FtInDims=%u)\n", index, (unsigned)FtInDims);
        }
        continue;
      }
      unsigned offset = kHalfDimensions * index;
      const unsigned maxOffset = kHalfDimensions * FtInDims;
      if (offset + kHalfDimensions > maxOffset) {
        static bool warned2 = false;
        if (!warned2) {
          warned2 = true;
          fprintf(stderr, "nnue: refresh_accumulator offset out of range offset=%u max=%u\n", offset, maxOffset);
        }
        continue;
      }
...
// Evaluation function
NOINLINE int nnue_evaluate_pos(Position *pos)
{
  int32_t out_value;
...
#define B(x) (buf.x)
  ...
  // Basic sanity checks to catch release-only issues early
  if (!pos || !pos->pieces || !pos->squares || !pos->nnue[0]) {
    fprintf(stderr, "nnue: invalid Position pointers in nnue_evaluate_pos\n");
    return 0;
  }
...
{
    LOG_DEBUG("Loading NNUE : " << evalFile);

    if (load_eval_file(evalFile)) {
        LOG_DEBUG("NNUE loaded !");
        return;
    }
    LOG_DEBUG("NNUE file not found!");
}
