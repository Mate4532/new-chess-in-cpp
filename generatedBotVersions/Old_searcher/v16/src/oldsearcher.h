...
namespace OldSearcher {
    ...
    private:
        ...
        std::shared_ptr<OldTT::TranspositionTable> tt;
    ...
    public:
        ...
        Searcher(int threads = 1) : board(), threads(threads) {
            tt = std::make_shared<OldTT::TranspositionTable>(TT_SIZE_MB);
            ClearHistory();
            ClearKillers();
            OldLMR::LMR::Init();
        ...
        Searcher(std::shared_ptr<OldTT::TranspositionTable> sharedTT) : board(), tt(sharedTT), threads(1) {
            ClearHistory();
            ClearKillers();
            OldLMR::LMR::Init();
            std::memset(nnue_state, 0, sizeof(nnue_state));
