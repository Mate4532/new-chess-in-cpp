...
int Searcher::negamax(int depth, int alpha, int beta, int ply) {
    ...
    int staticEval = currentSettings.worseEvaluationEnabled ? WorseEvaluation::Evaluation::EvaluatePos(board) : Evaluation::Evaluation::EvaluatePos(board, ply, nnue_state);

    if (!isPvNode && depth >= 3 && !inCheck && ply > 0 && abs(beta) < MATE_SCORE_BOUND) {
