...
Move Searcher::IterativeDeepening(bool silent) {
    ...
    if (!silent) LOG_DEBUG("Final Score: "
...
Move Searcher::GetRobotMove() {
    ...
    for (int i = 0; i < numThreads - 1; ++i) {
        helpers.emplace_back([this, i, board_snapshot]() mutable {
            Searcher helper(this->tt);
            ...
            helper.startTime = this->startTime;
            helper.hardTimeLimit = this->hardTimeLimit;
            helper.softTimeLimit = this->softTimeLimit;
